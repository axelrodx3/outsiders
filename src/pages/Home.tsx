...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const handleKey = (e: KeyboardEvent) => {
      if (e.key === 'Escape') {
        ...
        setVenueLightboxIndex(null)
      } else if (venueLightboxIndex !== null) {
        if (e.key === 'ArrowLeft') setVenueLightboxIndex((prev) => (prev === 1 ? 4 : prev! - 1))
        if (e.key === 'ArrowRight') setVenueLightboxIndex((prev) => (prev === 4 ? 1 : prev! + 1))
      } else if (lightboxIndex !== null) {
        if (e.key === 'ArrowLeft') setLightboxIndex((prev) => (prev === 1 ? 4 : prev! - 1))
        if (e.key === 'ArrowRight') setLightboxIndex((prev) => (prev === 4 ? 1 : prev! + 1))
      }
    }
  ...
  return (
    <div className="text-tavern-white">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[1, 2, 3, 4].map((i) => (
              <button
                key={i}
                type="button"
                onClick={() => setVenueLightboxIndex(i)}
      ...
      {venueLightboxIndex !== null && (
        <div
          className="fixed inset-0 z-[9999] bg-black/95 flex items-center justify-center p-4 animate-fade-in"
          onClick={() => setVenueLightboxIndex(null)}
          role="dialog"
          aria-modal="true"
          aria-label="Venue image lightbox"
        >
          <img
            src={`/gallery/venue-${venueLightboxIndex}.png`}
            alt={`Venue ${venueLightboxIndex}`}
            className="max-w-full max-h-[90vh] object-contain rounded relative z-0"
            onClick={(e) => e.stopPropagation()}
          />
          <button
            type="button"
            onClick={(e) => { e.preventDefault(); e.stopPropagation(); setVenueLightboxIndex(null) }}
          ...
          <button
            type="button"
            onClick={(e) => { e.preventDefault(); e.stopPropagation(); setVenueLightboxIndex((prev) => (prev === 1 ? 4 : prev! - 1)) }}
          ...
          <button
            type="button"
            onClick={(e) => { e.preventDefault(); e.stopPropagation(); setVenueLightboxIndex((prev) => (prev === 4 ? 1 : prev! + 1)) }}
      ...
      {lightboxIndex !== null && (
        ...
        >
          ...
          <button
            type="button"
            onClick={(e) => { e.preventDefault(); e.stopPropagation(); setLightboxIndex((prev) => (prev === 1 ? 4 : prev! - 1)) }}
            className="absolute left-2 top-1/2 -translate-y-1/2 z-[100] p-4 min-w-[48px] text-tavern-white hover:text-tavern-offwhite hover:bg-white/10 rounded text-4xl font-light transition-colors"
            aria-label="Previous image"
          >
            ‹
          ...
          <button
            type="button"
            onClick={(e) => { e.preventDefault(); e.stopPropagation(); setLightboxIndex((prev) => (prev === 4 ? 1 : prev! + 1)) }}
            className="absolute right-2 top-1/2 -translate-y-1/2 z-[100] p-4 min-w-[48px] text-tavern-white hover:text-tavern-offwhite hover:bg-white/10 rounded text-4xl font-light transition-colors"
            aria-label="Next image"
          >
            ›
